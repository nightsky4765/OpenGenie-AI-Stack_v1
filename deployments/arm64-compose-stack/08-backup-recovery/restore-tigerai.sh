...
restore_db() {
    ...
    if [ -f "$file" ]; then
        if docker ps | grep -q "$PG_CONTAINER"; then
            LOG "Dropping and re-creating database $PG_DB..."
            ...
            gunzip -c "$file" | docker exec -i "$PG_CONTAINER" psql -U "$PG_USER" "$PG_DB"
...
restore_data() {
    ...
    WARN "This will overwrite existing data. Services should be stopped."
