...
# Path: deployments/compose-stack/00-system-setup-gpu-driver-and-docker/resource/nvidia/install.sh
#
# 由同模組的 deploy.sh 透過 `tiger_res install.sh` 解析後 exec 起來，
# 也可以人工單獨執行（sudo bash resource/nvidia/install.sh）—— 兩種跑法的
# env 來源與結果相同，所以本檔自己也載入一次三層 env。
#
# ⚠️ 本檔與 resource/arm64/install.sh 目前只差「字串」與一處
#    `arch=$(dpkg --print-architecture)` vs `arch=arm64`（在 arm64 host 上兩者
#    求值相同）。搬移時刻意不合併，理由見 resource/arm64/install.sh 的檔頭。
#
# ⚠️ 這支腳本會改變 host 狀態（裝驅動 / 裝 Docker / 寫 sysctl / 寫 systemd unit /
#    改 gdm3），跑完要 reboot。安裝步驟本身與搬移前逐字相同，只有 env 載入段
#    與檔頭註解有改。
# =====================================================================
...
#    下移兩層到 resource/nvidia/，舊的相對路徑會指到 resource/ 底下（永遠不存在），
#    而 `if [ -f ... ]` 沒中就是靜默跳過 —— 結果是 NVIDIA_DRIVER_PACKAGE 之類的
#    設定被無聲忽略，只用 fallback 值，不會有任何錯誤訊息。
#    本檔位於 <stack>/<module>/resource/nvidia/，所以 ../.. 是模組目錄、
#    ../../.. 是 stack 根目錄。
#
# ⚠️ 這三個 env 檔都是 .gitignore 的（.gitattributes 的 EOL 正規化管不到），
#    所以仍要 `sed 's/\r$//'` 剝行尾 CRLF。用 source 而非舊版的
#    `export $(... | xargs)`：後者會被含空白／引號的值打爛。剝 CR 之後也就
#    不再需要舊版那段只挑特定變數名的 "Robust Variable Cleansing" 迴圈
#    （它靠 env | grep 名稱比對，本來就漏掉 NVIDIA_DRIVER_PACKAGE 等變數）。
...
# (sudo bash resource/nvidia/install.sh), where TIGER_PLATFORM is unset and
...
# ⚠️ 「System Foundation (NVIDIA RTX 5090 Ready)」是使用者辨識「跑對腳本」的字串，
#    不要洗掉。
LOG "============================================================="
LOG "✅ System Foundation (NVIDIA RTX 5090 Ready) Setup Complete."
LOG ""
LOG "Installed:"
LOG "  - NVIDIA Driver ($NVIDIA_DRIVER_PACKAGE / $NVIDIA_DKMS_PACKAGE / $NVIDIA_UTILS_PACKAGE)"
LOG "  - Docker CE + NVIDIA Container Toolkit"
