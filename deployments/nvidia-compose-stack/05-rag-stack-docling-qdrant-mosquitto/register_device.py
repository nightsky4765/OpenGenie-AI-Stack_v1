...
async def main():
    ...
    log.info("🚀 Registering device to MQTT broker %s:%s", MQTT_BROKER_HOST, MQTT_BROKER_PORT)
    ...
    try:
        async with Client(
            ...
            await client.publish("devices/register", payload=json.dumps(register_payload))
